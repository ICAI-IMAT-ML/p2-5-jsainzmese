...
class LogisticRegressor:
    ...
    def fit(
        ...
        for i in range(num_iterations):  # Fill the None here
            ...
            dw = (-1/m)*np.dot((y-y_hat),X)  # Derivative w.r.t. the coefficients(COEFICIENTES)
            db = -np.mean(y-y_hat)  # Derivative w.r.t. the intercept(SESGO)
    ...
    def lasso_regularization(self, dw, m, C):
        ...
        lasso_gradient = (C/m)*np.sign(self.weights) #valor absoluto
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        l1_gradient = (C/m)*np.sign(self.weights) #LASSO
        l2_ratio = 1-l1_ratio
        l2_gradient = (C/m)*self.weights #RIDGE
        elasticnet_gradient = l1_ratio * l1_gradient + l2_ratio * l2_gradient
    ...
    def log_likelihood(y, y_hat):
        ...
        loss = -(1/m) * np.sum(y*np.log(y_hat) + (1-y) * np.log(1-y_hat)) #Formmula from loss of log likelihood
        return loss
